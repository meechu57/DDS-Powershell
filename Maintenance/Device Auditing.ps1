...
# Audit Adobe running as administrator.
...
# Audit User Choice Protection Driver.
function Audit-UCPD {
...
function Audit-AutoRun {
  ...
  if ($autorunReg -and $autorunReg.NoDriveTypeAutorun -eq 0xFF) {
    if ($verbose -eq $true) {  Write-Host "Autorun and Autoplay is disabled on all drives." }
  	Add-Content -Path $logPath -Value "$(Get-Date -UFormat "%Y/%m/%d %T:") Autorun and Autoplay is disabled on all drives."
  	
  	return $true
  } else {
  	if ($verbose -eq $true) { Write-Host "Autorun and Autoplay is not disabled on all drives." }
...
# Audit Volume Shadow Copy Storage.
function Audit-VolumeShadowCopy {
  $vssOutput = vssadmin list shadowstorage /on=C: 2>&1
  if ($vssOutput -match "Maximum Shadow Copy Storage space:.*\((\d+(?:\.\d+)?%)\)") {
    $vssSpace = ($vssOutput -match "Maximum Shadow Copy Storage space:.*\((\d+(?:\.\d+)?%)\)")[1]

    if ($vssSpace -ne "10%") {
      if ($verbose -eq $true) { Write-Host "The current allocated size of the Volume Shadow Copy Storage isn't configured properly." }
      Add-Content -Path $logPath -Value "$(Get-Date -UFormat "%Y/%m/%d %T:") The current allocated size of the Volume Shadow Copy Storage isn't configured properly"

      return $false
    } else {
      if ($verbose -eq $true) { Write-Host "The current allocated size of the Volume Shadow Copy Storage is correctly configured." }
      Add-Content -Path $logPath -Value "$(Get-Date -UFormat "%Y/%m/%d %T:") The current allocated size of the Volume Shadow Copy Storage is correctly configured."

      return $true
    }
  }

  Write-Host "Failed to retrieve the current allocated size of the Volume Shadow Copy Storage."
  Add-Content -Path $logPath -Value "$(Get-Date -UFormat "%Y/%m/%d %T:") Failed to retrieve the current allocated size of the Volume Shadow Copy Storage."
  return $false
}
# The log path for this script.
$logPath = "C:\DDS\Logs\Audit.log"
...
function Audit-VolumeShadowCopy {
  $vssOutput = vssadmin list shadowstorage /on=C: 2>&1
  if ($vssOutput -match "Maximum Shadow Copy Storage space:.*\((\d+(?:\.\d+)?%)\)") {
    $vssSpace = (vssadmin list shadowstorage /on=C: | Where-Object { $_ -match 'Maximum Shadow Copy Storage space:' } | ForEach-Object { if ($_ -match '\(([^)]+)\)') { $matches[1] } })

    if ($vssSpace -ne "10%") {
      if ($verbose -eq $true) { Write-Host "The current allocated size of the Volume Shadow Copy Storage isn't configured properly. Current Value: $vssSpace" }
      Add-Content -Path $logPath -Value "$(Get-Date -UFormat "%Y/%m/%d %T:") The current allocated size of the Volume Shadow Copy Storage isn't configured properly. Current Value: $vssSpace"
...
$vssConfigured = [PSCustomObject]@{
  Name = "VSS"
  Value = Audit-VolumeShadowCopy
}
...
$auditingArray = @(
  ...
  $isoMountingDisabled, $networkAdapterConfigured, $usbControllerConfigured, $adobeEnabledAsAdmin, $ucpdConfigured, $autoRunConfigured, $vssConfigured
)
...
switch ($maintenanceOverride) {
  ...
  "627c3b85-6ade-48ee-b380-6fe757bf69e9" { $overrideResults += "VSS" }
