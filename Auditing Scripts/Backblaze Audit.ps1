...
function Get-FullBackupCount {
  ...
  $fullBackupCount = $Bucket | Where-Object { $_ -match '00-00\.MRIMG$' -or $_ -match '00-00\.MRIMGX$' } | Measure-Object | Select-Object -ExpandProperty Count
...
# Access Key and Key ID for Backblaze. Input your credentials here.
$id = ""
$key = ""

# Get into our account.
C:\DDS\b2v3-windows.exe account authorize $id $key | Out-Null
...
foreach ($bucket in $buckets) {
  ...
  if ($files) {
    ...
    foreach ($line in $files) {
      ...
      if ($fileName -match '^(.+)-(\d+)-\2\.MRIMG$' -or $fileName -match '^(.+)-(\d+)-\2\.MRIMGX$') {
        $backupID = $matches[1]
