...
function Process-ZipGroups {
  ...
  foreach ($group in $groups) {
    ...
    if ($totalUncompressedSize -gt 9GB -and $totalUncompressedSize -lt 10GB) {
      Write-Host "The [$($group.Name)] database size exceeds 9GB! Current size: $totalUncompressedSizeGB GB"
      exit 2
    } # Warn if uncompressed size exceeds 48GB
    elseif ($totalUncompressedSize -gt 48GB) {
      Write-Host "The [$($group.Name)] database size exceeds 48GB! Current size: $totalUncompressedSizeGB GB"
      exit 2
    }
  }
}
