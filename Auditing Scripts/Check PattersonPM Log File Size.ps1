...
if ($fileSize -ne $null) {
  ...
  Write-Host "The PattersonPM.log file does not exist."
  Add-Content -Path $logPath -Value "$(Get-Date -UFormat "%Y/%m/%d %T:") The PattersonPM.log file does not exist."
  
  exit 1
}

# For our condition tracking
if ($fileSize -gt 20000000){
  Add-Content -Path $logPath -Value "$(Get-Date -UFormat "%Y/%m/%d %T:") The PattersonPM.log file is larger than recommended please restart Patterson Server."
  Write-Host "The PattersonPM.log file is larger than recommended please restart Patterson Server."
}
