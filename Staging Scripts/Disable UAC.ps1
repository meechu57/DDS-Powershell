...
if ($uacRegKey -eq $null -or $uacRegKey.EnableLUA -ne 0) {
  try {
    ...
    Write-Host "Failed to add the EnableLUA Registry key and disable UAC: $_"
    Add-Content -Path $logPath -Value "$(Get-Date -UFormat "%Y/%m/%d %T:") Failed to add the EnableLUA Registry key and disable UAC: $_"
